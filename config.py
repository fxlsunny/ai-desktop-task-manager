"""
配置管理模块 - 负责读写 config.json，管理数据目录路径

路径策略（重要）：
- DEFAULT_DATA_DIR 为相对路径字符串 "data"，相对于本文件（config.py）所在目录。
  这样整个程序目录打包/拷贝到任何位置都能开箱即用，数据随程序走。
- cfg["data_dir"] 可以是相对路径（相对 _APP_DIR）或绝对路径（用户显式指定外部盘/
  共享目录时）。
- 对外统一通过 ensure_data_dir(cfg) 拿到 **绝对 Path**，其它模块无需关心相对/绝对。
- 保存时 `save()` 会自动把程序目录下的绝对路径转回相对路径，保证 config.json
  跨机器可移植。
"""
import json
import os
